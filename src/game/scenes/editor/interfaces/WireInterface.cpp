	...
	glm::dvec4 vport, NVGcontext* vg, GUIInput* gui_input, GUISkin* gui_skin)
{
	...
	{
		auto lambda = [this, vg, &gui_input, vport, &seen_positions, &rnd_idx, &new_wire, &machine_to_pos]
				(Machine* m, glm::dvec2 final_pos, glm::dvec3 pos, bool in_front)
		{
			return do_machine(vg, gui_input, m, final_pos, pos, vport, in_front, seen_positions,
							  rnd_idx,new_wire, machine_to_pos,
							  gui_input->mouse_blocked);
		};
		draw_machine_polygon(vg, pair.first, pair.second, &rnd_idx, vport, cu, lambda);
	}
...
void WireInterface::see_part(Part* p) 
{
	...
	for(Machine* mach : p->attached_machines)
	{
		mch.push_back(mach);
	}
	if(!mch.empty())
